...
class Painter():
    ...
    def get_src_and_canvas(self, args):
        ...
        src_img = args['image']
        imgcv = np.float32(args['image'])
        self.imgcv = cv2.cvtColor(imgcv, cv2.COLOR_RGB2GRAY) # [H, W]
    ...
    def draw_spline(self, K, r):
        ...
        trajectory = np.array(pts, dtype=np.float64)/max(self.H, self.W)*400
        trajectory /= 1000.0
        trajectory = rtb.mstraj(trajectory, dt=0.02, qdmax=0.25, tacc=0.05)
        trajectory = {'points': trajectory.q, 'width': r, 'color': 255*self.stroke_color}
