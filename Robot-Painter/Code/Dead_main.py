...
def convert_path(path):
    ...
    # flip image
    res[:,1] = res[:,1]-2*res[:,1]+img_width/compression_coeff
    return res
...
def draw_paths(paths):
    ...
    for path in paths:
        ...
        canvas.plot(path[:,0], path[:,1], linewidth=3, color='black')
    fig.canvas.draw()


canvas_size = 300
paths = []
result_path = 'Code/Result/'
filepath = ''
filename = ''
...
def img_to_square(img):
    ...
    return cv2.resize(square, (canvas_size, canvas_size))
